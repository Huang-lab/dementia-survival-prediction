...
###No cancer
noMI <- naccdata_1stVisit[naccdata_1stVisit$MYOINF == "0"]

#survival analysis model
fit.noMI <- survfit(Surv(time, NACCDIED) ~ CDRGLOB, data = noMI)
print(fit.noMI)
noMI.res.sum <- surv_summary(fit.noMI)
...
###primary cancer
MI <-  naccdata_1stVisit[naccdata_1stVisit$MYOINF == "1"]
#survival analysis model
fit.MI <- survfit(Surv(time, NACCDIED) ~ CDRGLOB, data = MI)
print(fit.MI)
MI.res.sum <- surv_summary(fit.MI)
...
cdr0 = naccdata_1stVisit[naccdata_1stVisit$CDRGLOB == "0" & 
                           (naccdata_1stVisit$MYOINF == "0" | naccdata_1stVisit$MYOINF == "1"  ) ]

#survival analysis model
fit.cdr0 <- survfit(Surv(time, NACCDIED) ~ MYOINF, data = cdr0)
print(fit.cdr0)
...
#survival analysis model
fit.cdr05 <- survfit(Surv(time, NACCDIED) ~ MYOINF, data = cdr05)
print(fit.cdr05)
...
#survival analysis model
fit.cdr1 <- survfit(Surv(time, NACCDIED) ~ MYOINF, data = cdr1)
print(fit.cdr1)
...
#survival analysis model
fit.cdr2 <- survfit(Surv(time, NACCDIED) ~ MYOINF, data = cdr2)
print(fit.cdr2)
...
#survival analysis model
fit.cdr3 <- survfit(Surv(time, NACCDIED) ~ MYOINF, data = cdr3)
print(fit.cdr3)

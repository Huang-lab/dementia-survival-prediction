...
#divide dataset by cancer status

###No cancer
nocancer <- naccdata_1stVisit[naccdata_1stVisit$CANCER == "0"]

  #survival analysis model
fit.nocancer <- survfit(Surv(time, NACCDIED) ~ CDRGLOB, data = nocancer)
print(fit.nocancer)
nocancer.res.sum <- surv_summary(fit.nocancer)
head(nocaner.res.sum)
ggsurvplot(fit.nocancer,
...
###primary cancer
pricancer <-  naccdata_1stVisit[naccdata_1stVisit$CANCER == "1"]
#survival analysis model
fit.pricancer <- survfit(Surv(time, NACCDIED) ~ CDRGLOB, data = pricancer)
print(fit.pricancer)
pricancer.res.sum <- surv_summary(fit.pricancer)
head(pricaner.res.sum)
ggsurvplot(fit.pricancer,
           pval = TRUE, conf.int = TRUE,
           # risk.table = TRUE, # Add risk table
           # risk.table.col = "strata", # Change risk table color by groups
           # linetype = "strata", # Change line type by groups
...
metacancer <-  naccdata_1stVisit[naccdata_1stVisit$CANCER == "2"]
#survival analysis model
fit.metacancer <- survfit(Surv(time, NACCDIED) ~ CDRGLOB, data = metacancer)
print(fit.metacancer)
metacancer.res.sum <- surv_summary(fit.metacancer)
head(metacaner.res.sum)
ggsurvplot(fit.metacancer,
           pval = TRUE, conf.int = TRUE,
           # risk.table = TRUE, # Add risk table
           # risk.table.col = "strata", # Change risk table color by groups
           # linetype = "strata", # Change line type by groups
           surv.median.line = "hv", # Specify median survival
           break.time.by = 1000,
           title = "Metastatic Cancer Patients",
           font.main = c(45,"bold","black"),
           font.x = c(35,"bold","black"),
           font.y = c(35,"bold","black"),
           font.tickslab =  c(35,"bold","black"),
...
cdr0 = naccdata_1stVisit[naccdata_1stVisit$CDRGLOB == "0" & 
                           (naccdata_1stVisit$CANCER == "0" | naccdata_1stVisit$CANCER == "1" | naccdata_1stVisit$CANCER == "2" ) ]

#survival analysis model
fit.cdr0 <- survfit(Surv(time, NACCDIED) ~ CANCER, data = cdr0)
print(fit.cdr0)
...
#survival analysis model
fit.cdr05 <- survfit(Surv(time, NACCDIED) ~ CANCER, data = cdr05)
print(fit.cdr05)
...
#survival analysis model
fit.cdr1 <- survfit(Surv(time, NACCDIED) ~ CANCER, data = cdr1)
print(fit.cdr1)
...
#survival analysis model
fit.cdr2 <- survfit(Surv(time, NACCDIED) ~ CANCER, data = cdr2)
print(fit.cdr2)
...
#survival analysis model
fit.cdr3 <- survfit(Surv(time, NACCDIED) ~ CANCER, data = cdr3)
print(fit.cdr3)
